...
async def on_ready() -> None:
    ...
    discord_client = bot
    if bot.user:
        logger.info(f"Logged in as {bot.user.name}")  # noqa: G004
...
async def call_tool(name: str, arguments: dict[str, Any]) -> list[TextContent]:
    ...
    assert discord_client is not None  # narrowing for type checker; guarded by decorator

    if name == "send_message":
        channel = await discord_client.fetch_channel(int(arguments["channel_id"]))
        message = await channel.send(arguments["content"])  # type: ignore[union-attr]
        ...
        messages = []
        async for message in channel.history(limit=limit):  # type: ignore[union-attr]
            reaction_data = []
            ...
            TextContent(
                ...
                + "\n".join(
                    [
                        ...
                        + "Reactions: "
                        + (", ".join(format_reaction(r) for r in m["reactions"]) if m["reactions"] else "No reactions")
                        for m in messages
                    ]
                ),
            )
        ...
        channel = await discord_client.fetch_channel(int(arguments["channel_id"]))
        message = await channel.fetch_message(int(arguments["message_id"]))  # type: ignore[union-attr]
    ...
    # Server Information Tools
        ...
        if not guild:
            raise ValueError(f"Guild {arguments['server_id']} not found in cache")
        channel_list = []
    ...
    # Role Management Tools
        ...
        if role is None:
            raise ValueError(f"Role {arguments['role_id']} not found in guild {guild.name}")
    ...
    # Channel Management Tools
        ...
        if "category_id" in arguments:
            ...
            if category is None:
                raise ValueError(f"Category {arguments['category_id']} not found in guild {guild.name}")
        ...
        channel = await discord_client.fetch_channel(int(arguments["channel_id"]))
        await channel.delete(reason=arguments.get("reason", "Channel deleted via MCP"))  # type: ignore[union-attr]
